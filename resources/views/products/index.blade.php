...
@section('content')
    <div class="flex items-center justify-between gap-3 mb-5">
        ...
        @can('products.create')
            <a href="{{ route('products.create') }}" class="rounded-lg lux-btn text-white text-sm font-medium px-4 py-2">+ New product</a>
        @endcan
    </div>

    <form method="GET" class="mb-4 grid sm:grid-cols-4 gap-3">
        <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search name / SKU…" class="lux-field px-3 py-2 text-sm">
        <select name="brand_id" class="lux-field px-3 py-2 text-sm">
        ...
        <button class="lux-field px-3 py-2 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">Filter</button>
    </form>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <x-act.view :href="route('products.show', $product)" />
                                    @can('products.update')<x-act.edit :href="route('products.edit', $product)" />@endcan
                                    @can('products.delete')<x-act.delete :action="route('products.destroy', $product)" confirm="Delete this product?" />@endcan
                                </div>
                            </td>
                        </tr>
                    @empty
