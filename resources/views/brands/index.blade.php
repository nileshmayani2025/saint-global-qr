...
@section('content')
    <div class="flex items-center justify-between gap-3 mb-5">
        ...
        @can('brands.create')
            <a href="{{ route('brands.create') }}" class="rounded-lg bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium px-4 py-2">+ New brand</a>
        @endcan
    </div>

    <form method="GET" class="mb-4 flex gap-3">
        <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search brands…" class="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm flex-1 max-w-xs">
