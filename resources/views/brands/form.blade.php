    ...
    <form method="POST" action="{{ $brand->exists ? route('brands.update', $brand) : route('brands.store') }}" class="mt-4 max-w-xl">
        ...
        <div class="lux-card p-6 space-y-5">
            <div>
                ...
                <input name="name" value="{{ old('name', $brand->name) }}" required class="w-full lux-field px-3.5 py-2.5">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1.5">Description</label>
                <textarea name="description" rows="3" class="w-full lux-field px-3.5 py-2.5">{{ old('description', $brand->description) }}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium mb-1.5">Sort order</label>
                    <input type="number" min="0" name="sort_order" value="{{ old('sort_order', $brand->sort_order ?? 0) }}" class="w-full lux-field px-3.5 py-2.5">
                </div>
                <div>
        ...
        <div class="mt-5 flex items-center gap-3">
            <button class="rounded-lg lux-btn text-white font-medium px-5 py-2.5">{{ $brand->exists ? 'Update' : 'Create' }} brand</button>
