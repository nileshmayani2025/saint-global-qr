...
@section('content')
    <div class="grid lg:grid-cols-3 gap-6" x-data="{ method: '{{ old('method', 'upi') }}' }">
        <div class="space-y-6">
            ...
            <form method="POST" action="{{ route('my.rewards.payout') }}" class="lux-card p-5 space-y-4">
                @csrf
                <h3 class="font-semibold">Request payout</h3>
                <div>
                    <label class="block text-sm mb-1">Amount (₹)</label>
                    <input type="number" step="0.01" min="1" name="amount" value="{{ old('amount') }}" required class="w-full lux-field px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm mb-1">Method</label>
                    <select name="method" x-model="method" class="w-full lux-field px-3 py-2">
                ...
                <div x-show="method === 'upi'">
                    ...
                    <input name="upi_id" value="{{ old('upi_id') }}" class="w-full lux-field px-3 py-2">
                </div>
                <div x-show="method === 'bank'" x-cloak class="space-y-3">
                    <input name="account_name" value="{{ old('account_name') }}" placeholder="Account holder name" class="w-full lux-field px-3 py-2">
                    <input name="account_number" value="{{ old('account_number') }}" placeholder="Account number" class="w-full lux-field px-3 py-2">
                    <input name="ifsc" value="{{ old('ifsc') }}" placeholder="IFSC code" class="w-full lux-field px-3 py-2">
                </div>
                <textarea name="note" rows="2" placeholder="Note (optional)" class="w-full lux-field px-3 py-2 text-sm">{{ old('note') }}</textarea>
                <button class="w-full rounded-lg lux-btn text-white font-medium py-2.5">Submit request</button>
            </form>
        </div>
