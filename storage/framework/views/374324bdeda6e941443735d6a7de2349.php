...
<div class="min-h-screen lg:flex">
    ...
    <aside x-show="sidebar" x-cloak x-transition:enter.duration.200ms
        ...
        <nav class="flex-1 overflow-y-auto px-3.5 py-4 space-y-1">
            ...
            <?php $__currentLoopData = $nav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(is_null($item['perm']) || auth()->user()->can($item['perm'])): ?>
                    <a href="<?php echo e(route($item['route'])); ?>" class="lux-nav flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium <?php echo e($isActive($item['route']) ? 'active' : ''); ?>">
                        <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="1.7" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="<?php echo e($item['icon']); ?>"/></svg>
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route('verify.form')); ?>" target="_blank" class="lux-nav flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium">
        ...
        <div class="p-4">
            <div class="rounded-2xl p-4 bg-white/5 border border-white/10 text-center">
                ...
                <div class="mt-0.5 text-[11px] text-gold"><?php echo e(auth()->user()->getRoleNames()->first() ?? 'member'); ?></div>
            </div>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="lux-topbar h-[70px] flex items-center gap-3 px-4 sm:px-6 sticky top-0 z-20">
            ...
            <h1 class="font-display font-bold text-xl"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h1>
            <div class="ml-auto flex items-center gap-2">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="flex items-center gap-2.5 pl-1.5 pr-3 py-1.5 rounded-xl border border-[var(--border)] hover:border-brand-400 transition">
                        <div class="w-8 h-8 rounded-lg grid place-items-center text-sm font-bold text-white" style="background:linear-gradient(135deg,#2ca0d4,#1b5a7c)"><?php echo e(strtoupper(substr(auth()->user()->name, 0, 1))); ?></div>
                        <span class="hidden sm:block text-sm font-semibold"><?php echo e(\Illuminate\Support\Str::of(auth()->user()->name)->words(2, '')); ?></span>
                    </button>
                    <div x-show="open" x-cloak x-transition @click.outside="open = false" class="lux-card absolute right-0 mt-2 w-60 py-2 text-sm">
                        <div class="px-4 py-2">
                            <div class="font-semibold truncate"><?php echo e(auth()->user()->email); ?></div>
                            <div class="text-xs text-[var(--muted)]"><?php echo e(auth()->user()->getRoleNames()->implode(', ') ?: 'No role'); ?></div>
                        </div>
                        <div class="lux-divider my-1"></div>
                        <a href="<?php echo e(route('my.rewards')); ?>" class="block px-4 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg mx-1">My Rewards</a>
                        <a href="<?php echo e(route('my.scans')); ?>" class="block px-4 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg mx-1">My Scans</a>
                        <div class="lux-divider my-1"></div>
                        <form method="POST" action="<?php echo e(route('logout')); ?>" class="mx-1"><?php echo csrf_field(); ?>
...
<?php echo $__env->make('partials.select2', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</body>
</html>
