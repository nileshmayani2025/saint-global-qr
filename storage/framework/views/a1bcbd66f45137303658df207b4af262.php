<?php $__env->startSection('title', 'Home'); ?>

<?php
    $actions = [
        ['route' => 'scan',        'label' => 'Scan &<br>Verify',      'icon' => 'M4 4h6v6H4V4zm10 0h6v6h-6V4zM4 14h6v6H4v-6zm10 3h3m3 0h.01M14 14h.01M17 20h3v-3'],
        ['route' => 'my.scans',    'label' => 'Product<br>History',    'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
        ['route' => 'my.rewards',  'label' => 'Rewards<br>History',    'icon' => 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zM5 12h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7'],
        ['route' => 'verify.form', 'label' => 'Enter<br>Code',         'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
    ];
?>

<?php $__env->startSection('content'); ?>
    
    <?php if($banners->isNotEmpty()): ?>
        <div x-data="{
                 active: 0,
                 count: <?php echo e($banners->count()); ?>,
                 timer: null,
                 start() { if (this.count > 1) this.timer = setInterval(() => this.active = (this.active + 1) % this.count, 5000) },
                 stop() { clearInterval(this.timer) },
                 go(i) { this.stop(); this.active = i; this.start() },
             }"
             x-init="start()" @mouseenter="stop()" @mouseleave="start()" class="mb-5">
            <div class="relative">
                <?php $__currentLoopData = $banners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $banner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    
                    <div x-show="active === <?php echo e($i); ?>" x-transition:enter.duration.400ms x-transition:enter.opacity.0
                         x-cloak class="relative overflow-hidden rounded-2xl">
                        <?php if($banner->image_path): ?>
                            <img src="<?php echo e(asset('media/'.$banner->image_path)); ?>" alt=""
                                 class="absolute inset-0 w-full h-full object-cover">
                            <div class="absolute inset-0 bg-gradient-to-r from-slate-950/85 via-slate-950/55 to-transparent"></div>
                        <?php endif; ?>

                        <div class="<?php echo \Illuminate\Support\Arr::toCssClasses([
                                'relative p-5 min-h-[150px] flex flex-col justify-center',
                                'bg-gradient-to-br from-brand-600 to-brand-800' => ! $banner->image_path,
                                'bg-slate-900' => (bool) $banner->image_path,
                            ]); ?>">
                            <h2 class="font-display text-lg font-bold text-white leading-snug max-w-[75%]"><?php echo e($banner->title); ?></h2>
                            <?php if($banner->subtitle): ?>
                                <p class="mt-1 text-sm text-white/75 max-w-[75%]"><?php echo e($banner->subtitle); ?></p>
                            <?php endif; ?>
                            <?php if($banner->button_label): ?>
                                <a href="<?php echo e($banner->link_url ?: route('scan')); ?>"
                                   class="mt-3 inline-flex w-max items-center gap-1.5 rounded-lg bg-white text-slate-900 text-xs font-semibold px-3.5 py-2 shadow hover:bg-white/90 transition">
                                    <?php echo e($banner->button_label); ?>

                                </a>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>

            <?php if($banners->count() > 1): ?>
                <div class="flex justify-center gap-1.5 mt-3">
                    <?php $__currentLoopData = $banners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $banner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button @click="go(<?php echo e($i); ?>)" aria-label="Slide <?php echo e($i + 1); ?>"
                                class="h-1.5 rounded-full transition-all"
                                :class="active === <?php echo e($i); ?> ? 'w-5 bg-brand-500' : 'w-1.5 bg-slate-300 dark:bg-slate-700'"></button>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            <?php endif; ?>
        </div>
    <?php endif; ?>

    
    <div class="grid grid-cols-3 gap-3">
        <div class="lux-card p-3.5 text-center">
            <div class="text-xl font-bold text-brand-600 dark:text-brand-400"><?php echo e(number_format($stats['total_scans'])); ?></div>
            <div class="mt-0.5 text-[11px] leading-tight text-[var(--muted)]">Products<br>Verified</div>
        </div>
        <div class="lux-card p-3.5 text-center">
            <div class="text-xl font-bold text-amber-600 dark:text-amber-400"><?php echo e(number_format($stats['points_earned'])); ?></div>
            <div class="mt-0.5 text-[11px] leading-tight text-[var(--muted)]">Reward<br>Points</div>
        </div>
        <div class="lux-card p-3.5 text-center">
            <div class="text-xl font-bold text-emerald-600 dark:text-emerald-400">₹<?php echo e(number_format($stats['balance'], 2)); ?></div>
            <div class="mt-0.5 text-[11px] leading-tight text-[var(--muted)]">vCash<br>Earned</div>
        </div>
    </div>

    
    <div class="lux-card mt-4 p-4">
        <div class="grid grid-cols-4 gap-2">
            <?php $__currentLoopData = $actions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $action): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route($action['route'])); ?>" class="flex flex-col items-center gap-2 group">
                    <span class="w-12 h-12 rounded-2xl grid place-items-center bg-brand-600 text-white shadow-sm group-hover:bg-brand-700 group-active:scale-95 transition">
                        <svg class="w-[22px] h-[22px]" fill="none" stroke="currentColor" stroke-width="1.7" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="<?php echo e($action['icon']); ?>"/>
                        </svg>
                    </span>
                    <span class="text-[11px] leading-tight text-center text-[var(--muted)] font-medium"><?php echo $action['label']; ?></span>
                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>

    
    <div class="mt-5">
        <div class="rounded-xl bg-brand-500/10 px-3.5 py-2 mb-3">
            <h2 class="text-xs font-bold tracking-wide text-brand-700 dark:text-brand-300">OUR PRODUCTS</h2>
        </div>

        <div class="grid grid-cols-2 gap-3">
            <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="lux-card p-3">
                    <?php if($product->image_path): ?>
                        <img src="<?php echo e(asset('media/'.$product->image_path)); ?>" alt=""
                             class="w-full aspect-square object-cover rounded-xl mb-2.5">
                    <?php else: ?>
                        <div class="w-full aspect-square rounded-xl mb-2.5 grid place-items-center bg-brand-500/10 text-brand-600 dark:text-brand-300 font-display font-bold text-2xl">
                            <?php echo e(strtoupper(substr($product->name, 0, 1))); ?>

                        </div>
                    <?php endif; ?>
                    <div class="text-sm font-semibold leading-tight line-clamp-2"><?php echo e($product->name); ?></div>
                    <div class="mt-1 flex items-center justify-between gap-2">
                        <span class="text-[11px] text-[var(--muted)]"><?php echo e($product->sku); ?></span>
                        <?php if((int) $product->reward_points > 0): ?>
                            <span class="text-[11px] font-semibold px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-700 dark:text-amber-300 shrink-0">
                                +<?php echo e($product->reward_points); ?> pts
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <p class="col-span-2 lux-card p-6 text-center text-sm text-[var(--muted)]">No products listed yet.</p>
            <?php endif; ?>
        </div>
    </div>

    
    <?php if($recentScans->isNotEmpty()): ?>
        <div class="mt-5">
            <div class="rounded-xl bg-brand-500/10 px-3.5 py-2 mb-3 flex items-center justify-between">
                <h2 class="text-xs font-bold tracking-wide text-brand-700 dark:text-brand-300">RECENT SCANS</h2>
                <a href="<?php echo e(route('my.scans')); ?>" class="text-[11px] font-semibold text-brand-600 dark:text-brand-400">View more</a>
            </div>
            <div class="lux-card divide-y divide-[var(--border)]">
                <?php $__currentLoopData = $recentScans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center justify-between gap-3 px-4 py-3">
                        <div class="min-w-0">
                            <div class="text-sm font-medium truncate"><?php echo e($log->product?->name ?? 'Product'); ?></div>
                            <div class="text-[11px] text-[var(--muted)]"><?php echo e($log->verified_at?->diffForHumans()); ?></div>
                        </div>
                        <?php if((int) $log->reward_points > 0): ?>
                            <span class="text-[11px] font-semibold px-2 py-1 rounded-full bg-amber-500/15 text-amber-700 dark:text-amber-300 shrink-0">
                                +<?php echo e($log->reward_points); ?> pts
                            </span>
                        <?php endif; ?>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
    <?php endif; ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.consumer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp\htdocs\Saint Global\resources\views/dashboard-consumer.blade.php ENDPATH**/ ?>